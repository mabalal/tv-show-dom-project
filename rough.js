...
//   let liveSearchBox = document.getElementById("search");
//   let clearButton = document.getElementById("clearButton");
//   let getAllEpisodes = document.getElementById("showsAllButton");
//   let selectTag = document.getElementById("episode");
//   let showsTag = document.getElementById("shows");
//   let showTitle = document.getElementById("showTitle");
//   let selectShow = document.getElementById("selectShow");
//   let startSnowNum = 82;
//   let tvShows = getAllEpisodes();
    ...
    fetch(`https://api.tvmaze.com/shows/${startShowNum}/episodes`)
      ...
      .then((data) => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          let modifiedSummary = `${tvShows[i].summary}`.substring(
            3,
            tvShows[i].summary.length - 4
          );
          ...
          fetch(`https://api.tvmaze.com/shows/${showNum}/episodes`)
            ...
            .then((dataApi) => {
              ...
              for (let i = 0; i < dataApi.length; i++) {
                ...
                cardImage.style.borderRadius = "5px";
                let modifiedSummary = `${dataApi[i].summary}`.substring(
                  3,
                  dataApi[i].summary.length - 4
                );
                cardSummary.textContent = modifiedSummary;
              ...
              function searchHandler2() {
                ...
                let filteredEpisodes = dataApi.filter(({ name, summary }) => {
                  return (
                    ...
                    summary.toLowerCase().includes(searchResult)
                  );
                });
                ...
                for (let i = 0; i < filteredEpisodes.length; i++) {
                  ...
                  cardImage.style.borderRadius = "5px";
                  let modifiedSummary =
                    `${filteredEpisodes[i].summary}`.substring(
                      3,
                      filteredEpisodes[i].summary.length - 4
                    );
                  cardSummary.textContent = modifiedSummary;
              ...
              function selectEpisode2(x) {
                ...
                //ADD CONTENT TO ELEMENTS
                title.textContent = `${dataApi[x.target.value].name} - S0${
                  dataApi[x.target.value].season
                }E0${dataApi[x.target.value].number}`;
                cardImage.src = `${dataApi[x.target.value].image.medium}`;
                cardImage.style.borderRadius = "5px";

                let modifiedSummary = `${
                  dataApi[x.target.value].summary
                }`.substring(3, dataApi[x.target.value].summary.length - 4);
                cardSummary.textContent = modifiedSummary;
        ...
        function searchHandler() {
          ...
          for (let i = 0; i < filteredEpisodes.length; i++) {
            ...
            let modifiedSummary = `${filteredEpisodes[i].summary}`.substring(
              3,
              filteredEpisodes[i].summary.length - 4
            );
            cardSummary.textContent = modifiedSummary;
        ...
        function selectEpisode(x) {
          ...
          //ADD CONTENT TO ELEMENTS
          title.textContent = `${data[x.target.value].name} - S0${
            data[x.target.value].season
          }E0${data[x.target.value].number}`;
